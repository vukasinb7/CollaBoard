...
pub async fn create_invitation(ctx: Ctx, Extension(pool): Extension<DbPool>, Json(payload): Json<InvitationPayload>) -> Result<Json<Invitation>, Error> {
    ...
    let invited_user = users::table
        .filter(users::email.eq(&payload.user_email))
        .first::<User>(&mut connection)
        .map_err(|_| Error::UserNotFound)?;

    let board = boards::table
        .filter(boards::id.eq(payload.board_id))
    ...
    let rendered_html = html_content.replace("{{board_name}}", &*board.name.clone())
        ...
        .replace("{{board_role}}", match new_invitation.role {
            2 => "Owner",
            1 => "Editor",
            _ => "Viewer"
        })
    ...
    let email = Message::builder()
        ...
        .to(invited_user.email.parse().unwrap())
        .subject("Board Invitation")
        .header(ContentType::TEXT_HTML)
        .body(rendered_html)
        .unwrap();

    let creds = Credentials::new((*utils::constants::SMTP_USERNAME).clone(),
                                 (*utils::constants::SMTP_PASSWORD).clone());

    let mailer = SmtpTransport::relay("smtp.gmail.com").unwrap()
        .credentials(creds)
        .build();

    mailer.send(&email.clone()).map_err(|_| Error::FailSendingMail).expect("Fail sending mail");
...
pub async fn accept_invitation(ctx: Ctx, Extension(pool): Extension<DbPool>, Path(invitation_code): Path<String>) -> Result<Json<Value>, Error> {
    ...
    let invitation: Invitation = invitations::table
        .filter(invitations::code.eq(&invitation_code))
    ...
    let _ = diesel::delete(invitations::table
        .filter(invitations::code.eq(invitation.code)))
        .execute(&mut connection)
...
pub async fn change_permission(ctx: Ctx, Extension(pool): Extension<DbPool>, Json(payload): Json<InvitationPayload>) -> Result<Json<Value>, Error> {
    ...
    diesel::update(permissions::table
        .filter(permissions::board_id.eq(&board.id)
            .and(permissions::user_id.eq(&invited_user.id))))
...
pub async fn delete_permission(ctx: Ctx, Extension(pool): Extension<DbPool>, Query(payload): Query<DeletePermissionParams>) -> Result<Json<Value>, Error> {
    ...
    diesel::delete(permissions::table
        .filter(permissions::user_id.eq(&issued_user.id).and(permissions::board_id.eq(&board.id))))
        .execute(&mut connection)
        .map_err(|_| Error::FailDeleteDB)?;
